...
if TYPE_CHECKING:
    from .terrain_importer_cfg import TerrainImporterCfg, VariedGridTerrainGeneratorCfg
...
class TerrainImporter:
    ...
    def __init__(self, cfg: VariedGridTerrainGeneratorCfg):
...
class VariedGridTerrainImporter(TerrainImporter):
    """A class to handle varied grid terrain meshes and import them into the simulator.
    This class extends the TerrainImporter class to handle varied grid terrains. The varied terrains are
    generated by specifying different "table" locations for each sub-terrain. The table locations are 
    specified as a list of 3D coordinates.
    ...
    def __init__(self, cfg: TerrainImporterCfg):
        ...
        if self.cfg.terrain_type == "generator":
            ...
            if self.cfg.terrain_generator is None:
                raise ValueError("Input terrain type is 'generator' but no value provided for 'terrain_generator'.")
            # generate the terrain
            terrain_generator = VariedGridTerrainGenerator(cfg=self.cfg.terrain_generator, device=self.device)
            self.import_mesh("terrain", terrain_generator.terrain_mesh)
            # configure the terrain origins based on the terrain generator
            self.configure_env_origins(terrain_generator.terrain_origins)
            # refer to the flat patches
    ...
    def configure_env_origins(self, origins: np.ndarray | None = None):
        ...
        This function configures the environment origins based on the sub-terrain origins. It sets the 
        environment origins to the sub-terrain origins. Throws an error if the number of environments is
        different than the number of sub-terrains.
        ...
        if origins is not None:
            ...
            self.terrain_origins = origins.to(self.device, dtype=torch.float)
            # compute environment origins
            self.env_origins = self._compute_env_origins_grid(self.cfg.num_envs, self.terrain_origins)
        else:
            self.terrain_origins = None
            # check if env spacing is valid
            if self.cfg.env_spacing is None:
    ...
    def _compute_env_origins_grid(self, num_envs: int, origins: torch.Tensor) -> torch.Tensor:
        """Set the origins of the environments defined by the sub-terrains origins."""
        # extract number of rows and cols
        num_rows, num_cols = origins.shape[:2]
        ...
        # if number of environments is different than number of sub-terrains, throw an error
        if num_envs != num_rows * num_cols:
            raise ValueError(f"Number of environments must be equal to the number of sub-terrains ({num_rows} x {num_cols}).")

        env_origins = origins.view(-1, 3)
        return env_origins