...
from omni.isaac.lab.terrains.config.hsrb_reach import HSRB_REACH_TERRAINS_CFG, HSRB_STUDENT_REACH_TERRAINS_CFG  # isort: skip
...
class StudentCurriculumCfg(CurriculumCfg):
    ...
    ### Put the reward weights to what they are at the end of the teacher curriculum ###
    ### Means reward weightings are the same as the teacher curriculum, so the student performance can be compared to the teacher ###
    action_rate = CurrTerm(
        func=mdp.modify_reward_weight, params={"term_name": "action_rate", "weight": -0.005, "num_steps": 96} # Update after 1 iterations
    )

    joint_vel = CurrTerm(
        func=mdp.modify_reward_weight, params={"term_name": "joint_vel", "weight": -0.005, "num_steps": 96}
    )

    end_effector_position_tracking = CurrTerm(
        func=mdp.modify_reward_parameters, params={"term_name": "end_effector_position_tracking", "parameters": {"frame_name": "ee_frame", "command_name": "ee_pose", "position_threshold": 0.10}, "num_steps": 96} # Update after 1 iterations
    )

    termination_penalty = CurrTerm(
        func=mdp.modify_reward_weight, params={"term_name": "termination_penalty", "weight": -50.0, "num_steps": 96} # 1 iterations
    )

    ee_acc = CurrTerm(
        func=mdp.modify_reward_weight, params={"term_name": "ee_acc", "weight": -0.005, "num_steps": 96} # 1 iterations
    )
...
@configclass
class HSRBReachStudentEnvCfg(MobileReachEnvCfg):
    def __post_init__(self):
        ...
        self.scene.terrain = TerrainImporterCfg(
            ...
            terrain_generator=HSRB_STUDENT_REACH_TERRAINS_CFG,
        ...
        self.actions.base_action = mdp.MimicPureJointVelocityActionCfg(
            asset_name="robot", joint_names=["joint_x", "joint_y", "joint_rz"], use_default_offset=True, scale=1.0, debug_vis=False
        )
        ...
        #     prim_path="{ENV_REGEX_NS}/Robot/arm_lift_link", update_period=0.0, history_length=contact_history_length, debug_vis=False, filter_prim_paths_expr=arm_contact_force_prim_paths # ["/World/ground/terrain/mesh", "{ENV_REGEX_NS}/Robot/base.*/collisions/mesh.*", "{ENV_REGEX_NS}/Robot/wrist_roll_link", "{ENV_REGEX_NS}/Robot/hand.*", "{ENV_REGEX_NS}/Robot/torso_lift_link", "{ENV_REGEX_NS}/Robot/head.*"], # {ENV_REGEX_NS}/Robot/base_f_bumper_link
        # )
...
@configclass
class HSRBReachStudentEnvCfg_PPO(HSRBReachStudentEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
...
@configclass
class HSRBReachStudentEnvCfg_PLAY(HSRBReachStudentEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
...
@configclass
class HSRBReachStudentEnvCfg_PPO_PLAY(HSRBReachStudentEnvCfg_PPO):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
        # make a smaller scene for play
